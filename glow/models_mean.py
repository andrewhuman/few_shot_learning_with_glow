...
class Glow(nn.Module):
    ...
    def __init__(self, hparams,test_class_index,is_mean,K,y_classes,is_split_2d = False,is_prior_total= False,is_prior_wh= False,z_add_random = True,arc_loss = False,m=0.5):
        ...
        if is_mean:
            self.is_prior_wh = False

        if hparams.Glow.learn_top:
            C = self.flow.output_shapes[-1][1]

            if is_mean :
                self.learn_top = modules.LinearZeros(C * 2, C * 2)
            else:
        ...
        if hparams.Glow.y_condition:
            ...
            if self.arc_loss :
                self.metric_fc = ArcMarginProduct(self.C_project_class, self.y_classes, s=30, m=m)
            else:
    ...
    def prior_ismean(self ,shape , y_onehot=None):
        # print('prior_h size = ',self.prior_h.size())
        # print('learn_top size = ', self.learn_top.size())
        # print('project_ycond size = ', self.project_ycond.size())
        self.prior_h = torch.zeros(shape).cuda()  # if gpu or cpu
        ...
        if self.hparams.Glow.y_condition:
            ...
            # print('y_condition h size = ', h.size())
        return thops.split_linear_feature(h, "split")
    ...
    def class_flow(self,z_mean,y_one_shot = None):

        if self.hparams.Glow.y_condition:
            if not self.is_mean:
                z_mean = z_mean.view(-1, self.C_project_class)  # this have bugs ?
            # else:
    ...
    def normal_flow(self, x, y_onehot):
        ...
        if self.is_mean:
            z = z.mean(2).mean(2)
            mean, logs = self.prior_ismean([z.size()[0], z.size()[1] * 2], y_onehot)
