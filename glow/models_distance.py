...
class Glow(nn.Module):
    ...
    def __init__(self, hparams,test_class_index,is_mean,K,y_classes,L  =3,is_split_2d = False,is_prior_total= False,
                 is_prior_wh= False,z_add_random = True,arc_loss = False,C_project_class =64*4*4,split2d_type=1,squeeze_single = False):
        ...
        if hparams.Glow.y_condition:
            ...
            # # elif self.is_drop_z:
            # #     self.C_project_class = self.flow.output_shapes[-1][2] * self.flow.output_shapes[-1][3]
            # else :
            #     self.C_project_class = C * self.flow.output_shapes[-1][2] * self.flow.output_shapes[-1][3]
            self.C_project_class = C_project_class

            if self.arc_loss :
                self.metric_fc = ArcMarginProduct(self.C_project_class, self.y_classes, s=30, m=0.5)
            else:
    ...
    def prior(self ,shape , y_onehot=None):
        ...
        if self.hparams.Glow.learn_top:
            ...
            # print('after learn_top ,torch.sum(h) = ', torch.sum(h))  # sum h  != 0
        ...
        if self.hparams.Glow.y_condition:
            ...
            # print('after y_condition ,torch.sum(yp ,) = ', torch.sum(yp)) # sum yp  != 0
    ...
    def prior_t(self ,shape):
        ...
        if self.hparams.Glow.y_condition:
            ...
            # print('y_condition h size = ', h.size())
        return thops.split_feature_weight_diff(h, "split")

    def forward(self, x=None, y_onehot=None, z=None,
                eps_std=None, reverse=False,weight_prior = 1):
        if not reverse:
            return self.normal_flow(x, y_onehot,weight_prior =weight_prior)
        else:
    ...
    def normal_flow(self, x, y_onehot,weight_prior ):
        ...
        if self.training:
            ...
            loss_mean_log =  modules.GaussianDiag.logp(mean, logs, z) * weight_prior
            # print('prior mean  log var error / pixels =', torch.mean(sum).data / float(pixels))
            # objective -= torch.abs(loss_mean_log)
            objective += loss_mean_log
